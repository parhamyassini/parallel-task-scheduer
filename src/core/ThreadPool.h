...
//                            task = std::move(this->jobQueue.dequeue());
                            task = std::move(this->jobQueue.dequeue());
...
//                            jobQueue.pop();
    ...
    auto add_job(Function&& f, Args&&... args) -> future<typename std::result_of<Function(Args...)>::type>
    {
...
private:
    ...
    NonBlockingQueue<function<void()>> jobQueue;
